...
// ── Historial de pedidos del cliente (antes vivía en la página
//    /mis-pedidos, hoy es una sección del modal de cuenta). Helpers movidos
//    tal cual desde MisPedidosPage.jsx, sin reescribir. Los colores por
//    estado son semánticos (uno por estado, iguales en claro y oscuro).
const estadoColorPedido = { pendiente_verificacion: '#F57F17', pendiente: '#f59e0b', en_proceso: '#3b82f6', en_camino: '#00838F', entregado: '#6b7280', cancelado: '#ef4444', anulado: '#ef4444' };
const extrasDeItem = (item) => [
  ...(Array.isArray(item.adiciones) ? item.adiciones : []),
  ...(Array.isArray(item.toppings) ? item.toppings : []),
].map(e => (typeof e === 'string'
  ? { nombre: e, precio: 0 }
  : { nombre: e.nombre || e.name || 'Extra', precio: Number(e.precio || e.price || 0) }));
...
function PasarelaPago({ cart, total, cliente, onClose, onSuccess, onCerrarFinal }) {
  ...
  // El texto del método "Efectivo" cambia según cómo reciba el pedido el
  // cliente: si es a domicilio paga al recibirlo, si lo recoge paga en caja
  // al recogerlo. Nunca "pagar al retirar" a secas (ambiguo para domicilios).
  const efectivoTexto = tipoEntrega === 'domicilio'
    ? 'Pagas en efectivo al recibir tu pedido'
    : 'Pagas en efectivo al recoger tu pedido';
  const METODOS = [
    { id:'nequi',        label:'Nequi',            titular:'Café Don Berna S.A.S.', num:'300 000 0000', tipoCuenta:null, icon:'NQ' },
    { id:'transferencia', label:'Llave Bancolombia', titular:'Café Don Berna S.A.S.', num:'300 000 0000', tipoCuenta:'Ahorros', icon:'LB' },
    { id:'efectivo',     label:'Efectivo',         titular:null, num:efectivoTexto, tipoCuenta:null, icon:'EF' },
  ];
  ...
  // Detalle COMPLETO del pedido para el resumen en pantalla (pasos 3 y 4):
  // cada producto con su precio unitario, sus toppings/adiciones elegidos con
  // su precio, la cantidad y el subtotal de la línea.
  const resumenItems = cart.map(i => {
    const qty = i.qty || 1;
    const precioUnitBase = i.precio || i.price || 0;
    const extras = [
      ...((i.toppings || []).map(t => ({ nombre: t.nombre, precio: t.precio || 0 }))),
      ...((i.adiciones || []).map(a => ({ nombre: a.nombre, precio: a.precio || 0 }))),
    ];
    const precioUnit = precioUnitBase + extras.reduce((s, e) => s + e.precio, 0);
    return { nombre: i.nombre || i.name, qty, precioUnitBase, extras, precioUnit, subtotal: precioUnit * qty };
  });

  // Canal de contacto/soporte — NO para enviar el comprobante de pago (ese
  // siempre se sube en la plataforma). Solo por si el cliente necesita ayuda.
  const abrirContacto = () => {
    const msg = encodeURIComponent(
      `Hola, necesito ayuda con mi pedido ${numeroPedido} de Café Don Berna.`
    );
  ...
  const confirmar = async () => {
    ...
    // Para pagos con comprobante (Nequi / Llave Bancolombia) el pantallazo se
    // sube SIEMPRE en la plataforma y debe pasar la verificación del total.
    if (metodo !== 'efectivo' && (!archivo || !ocrOk)) return;
    ...
    const comprobanteInfo = archivo ? archivo.name : null;
    try {
      ...
      await pedidosService.create({
        ...
        comprobanteImg: metodo !== 'efectivo' ? comprobanteB64 : null,
        comprobanteVerificadoOcr: metodo !== 'efectivo' ? ocrOk : false,
        comprobanteTotalOcr: metodo !== 'efectivo' ? ocrTotalDetectado : null,
        origen: 'landing',
          ...
          <div className="pay-success">
            ...
            <p>{metodo === 'efectivo'
              ? (tipoEntrega === 'domicilio'
                  ? 'Tu pedido fue registrado. Pagas en efectivo al recibir tu pedido en la dirección de entrega.'
                  : `Tu pedido fue registrado. Pásate por ${localSel ? localSel.nombre : 'el local'} a recogerlo y pagas en efectivo en caja al momento de recogerlo.`)
            ...
            {step === 3 && (
              <div className="pay-body">
                ...
                <div className="pay-cart-summary">
                  ...
                  {resumenItems.map((it,idx) => (
                    <div key={idx} className="pay-detail-item">
                      <div className="pay-detail-item__head">
                        <span className="pay-detail-item__name"><b>{it.qty}×</b> {it.nombre}</span>
                        <span className="pay-detail-item__sub">{fmt(it.subtotal)}</span>
                      </div>
                      <div className="pay-detail-item__line">
                        <span>Producto{it.qty>1?` (${fmt(it.precioUnitBase)} c/u)`:''}</span>
                        <span>{fmt(it.precioUnitBase * it.qty)}</span>
                      </div>
                      {it.extras.map((e,j) => (
                        <div key={j} className="pay-detail-item__line pay-detail-item__line--extra">
                          <span>+ {e.nombre}</span>
                          <span style={{color: e.precio===0 ? '#4CAF50' : 'inherit'}}>{e.precio>0 ? `+${fmt(e.precio * it.qty)}` : 'Gratis'}</span>
                        </div>
                      ))}
                    </div>
                  ))}
            ...
            {step === 5 && (
              <div className="pay-body">
                ...
                <p style={{fontSize:13,color:"#888",marginBottom:16}}>
                  Adjunta aquí el pantallazo de la transacción de {metodoSel?.label}. El comprobante se sube <strong>siempre por esta plataforma</strong> — así podemos verificarlo y empezar a preparar tu pedido.
                </p>
                ...
                <div className="pay-comp-option pay-comp-option--sel">
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <span style={{fontSize:22}}>📎</span>
                    <div>
                      <div style={{fontWeight:700,fontSize:13}}>Subir imagen del comprobante</div>
                ...
                <div className="pay-contacto">
                  <span>¿Tienes un problema con tu pago o tu pedido?</span>
                  <button type="button" className="pay-contacto__btn" onClick={abrirContacto}>
                ...
                <div style={{display:"flex",gap:12,marginTop:16}}>
                  ...
                  <button className="lx-btn" style={{flex:1,justifyContent:"center"}}
                    disabled={loading || ocrProcesando || !archivo || !ocrOk}
                    onClick={confirmar}>
...
export default function Landing() {
  ...
  // (El menú desplegable del usuario se eliminó: la píldora abre directo el
  //  modal de cuenta y "Cerrar sesión" es un icono propio en la barra.)
  ...
  // (El puente por sessionStorage con la antigua página /mis-pedidos se
  // eliminó: el historial ahora es una sección del modal de cuenta y llama
  // directo a volverAComprar / abrirSolicitudDevolucion.)
  ...
  const verFacturaPedido = (pedido) => {
    ...
    const estadoTxt = etiquetaEstadoPedido(pedido.estado, pedido.tipo);
    w.document.write(`
  ...
  // Modal unificado de cuenta (Mi perfil / Historial / Editar perfil). Abre
  // SIEMPRE en "Mi perfil"; el usuario cambia de sección con las pestañas de
  // arriba sin cerrar el modal ni cambiar de ruta. Sin animación de
  // transición. (La carga de datos de abajo no se toca.)
  ...
  const handleEditPerfil = async e => {
    ...
    try {
      ...
      // Punto 4: reflejar los cambios en la sección "Mi perfil" del mismo
      // modal sin cerrarlo/reabrirlo. Mismo endpoint (ya se llamó arriba);
      // aquí solo se mezcla en el estado local lo que edita el formulario.
      setClienteData(prev => ({
        ...(prev || {}),
...
const handleLogin = async e => {
  ...
  if (adminResult.success) {
    ...
    playTransition(() => { setModal(null); navigate(adminResult.redirectTo || '/admin'); }, { message: '¡Bienvenido!' });
    return;
  }
  ...
  const handleRegister = async e => {
    ...
    if (regData.numeroDoc.length > 10) { setAuthError("El número de documento no puede tener más de 10 dígitos."); return; }
      ...
      <nav className={`lx-nav ${scrollY>60?"lx-nav--solid":""}`}>
        <div className="lx-nav__in">
          ...
          <div className="lx-nav__right">
            {clienteSession ? (
              <>
                {/* La píldora ya no despliega menú (se quitó, y con él la
                    flecha): al hacer clic abre directo el modal de cuenta en
                    la sección "Mi perfil". */}
                <button
                  type="button"
                  className="lx-user-info"
                  onClick={() => abrirPerfil()}
                  title="Mi cuenta"
                  aria-label={`Mi cuenta de ${clienteSession.nombre}`}
                >
                  <span className="lx-user-avatar">{clienteSession.nombre.charAt(0).toUpperCase()}</span>
                  <span className="lx-user-text">
                    <span className="lx-user-welcome">Hola,</span>
                    <span className="lx-user-name">{clienteSession.nombre.split(" ")[0]}</span>
                  </span>
                </button>
                {/* Cerrar sesión: icono suelto en la barra (junto a tema y
                    carrito). Mismo icono que el sidebar del panel
                    (Layout.jsx icons.logout); la flecha apunta hacia afuera
                    de la puerta = salir. Mantiene el flujo de cierre
                    existente (modal de confirmación → handleLogout). */}
                <button
                  type="button"
                  className="lx-logout-icon"
...
{bgImg && <div className="lx-hero__product-frame"><img src={bgImg} alt={pd.nombre}/></div>}
              ...
              <form className="lx-form" onSubmit={handleRegister}>
                <div className="lx-form__2">
                  ...
                  <div className="lx-field">
                    ...
                    {/* 3 — máximo 10 dígitos, bloqueado en tiempo real (no se
                        puede escribir el 11°) además de la validación al enviar. */}
                    <input type="text" inputMode="numeric" maxLength={10} placeholder="Solo números (máx. 10 dígitos)"
                      value={regData.numeroDoc}
                      onChange={e=>setRegData({...regData,numeroDoc:e.target.value.replace(/[^0-9]/g,'').slice(0,10)})}/>
                    <span className="lx-field__hint" style={{fontSize:11,color: regData.numeroDoc.length===10 ? '#E53935' : 'var(--lx-muted)'}}>
                      {regData.numeroDoc.length}/10 dígitos{regData.numeroDoc.length===10 ? ' — máximo alcanzado' : ''}
                    </span>
                  </div>
                </div>
      ...
      {modal === "perfil" && clienteSession && (() => {
        const cliente = clienteData;
        // Modal único de cuenta con 3 secciones (pestañas arriba). Ver el
        // porqué de "pestañas" (vs apilado) en el resumen de la tarea.
        const TABS = [
        ...
        return (
          <div className="lx-modal-mask" onClick={() => setModal(null)}>
            <div className="lx-modal lx-perfil-modal" onClick={e => e.stopPropagation()}>
              <button className="lx-modal__x" onClick={() => setModal(null)}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>

              <div className="lx-perfil-head">
                <div className="lx-perfil-avatar">{clienteSession.nombre.charAt(0).toUpperCase()}</div>
                <div style={{minWidth:0}}>
                  <div className="lx-perfil-name">{clienteSession.nombre}</div>
                  <div className="lx-perfil-email">{cliente?.correo || clienteSession.correo}</div>
                  <div className="lx-perfil-active">● Cliente activo</div>
                </div>
              </div>

              <div className="lx-modal__tabs lx-perfil-tabs">
                {TABS.map(t => (
                  <button key={t.id} className={perfilTab === t.id ? 'on' : ''} onClick={() => setPerfilTab(t.id)}>{t.label}</button>
                ))}
              </div>

              <div className="lx-perfil-body">
              {perfilTab === "info" && (
              ...
              {/* Historial de pedidos — contenido movido desde la antigua
                  página /mis-pedidos. Reutiliza `pedidosCliente` (ya se
                  carga en Landing con el mismo filtro/orden) y las funciones
                  verFacturaPedido / volverAComprar / abrirSolicitudDevolucion
                  que ya existen aquí. */}
              {perfilTab === "historial" && (
                pedidosCliente.length === 0 ? (
                  <div style={{ textAlign:'center', padding:'44px 0', color:'var(--lx-muted)' }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                    <div style={{ fontSize:13, color:'var(--lx-muted)' }}>{pedidosCliente.length} pedido{pedidosCliente.length !== 1 ? 's' : ''} en total</div>
                    {pedidosCliente.map(p => {
                      const estNorm = normalizarEstadoPedido(p.estado);
                      const color = estadoColorPedido[estNorm] || '#6b7280';
                      const productos = Array.isArray(p.productos) ? p.productos : (Array.isArray(p.items) ? p.items : []);
                      const motivoRechazo = p.comprobante_motivo_rechazo || p.comprobanteMotivoRechazo || null;
                      const direccionAlt = p.direccion_alternativa || p.direccionAlternativa || null;
                      return (
                        <div key={p.id} style={{ border:'1px solid var(--lx-border)', borderRadius:14, padding:'16px 18px', background:'rgba(128,128,128,.05)' }}>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                            <span style={{ fontWeight:800, fontSize:14, color:'var(--lx-text)' }}>Pedido #{p.id}</span>
                            <span style={{ fontSize:11, fontWeight:800, padding:'4px 12px', borderRadius:100, background:color+'22', color }}>{etiquetaEstadoPedido(p.estado, p.tipo)}</span>
                          </div>
                          {productos.length > 0 && (
                            <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:12 }}>
                              {productos.map((x, i) => {
                                ...
                                const sub = x.precioTotal ? Number(x.precioTotal) * cant
                                          ...
                                          : (Number(x.precio || 0) + extras.reduce((s, e) => s + e.precio, 0)) * cant;
                                return (
                                  <div key={i} style={{ display:'flex', flexDirection:'column', gap:3, paddingBottom:8, borderBottom: i < productos.length - 1 ? '1px dashed var(--lx-border)' : 'none' }}>
                                    <div style={{ display:'flex', justifyContent:'space-between', gap:10 }}>
                                      <span style={{ fontSize:13, fontWeight:700, color:'var(--lx-text)' }}>
                                        <span style={{ color:'var(--lx-green)', fontWeight:800 }}>{cant}×</span> {nombre}
                                      </span>
                                      {sub > 0 && <span style={{ fontSize:13, fontWeight:700, color:'var(--lx-text)', whiteSpace:'nowrap' }}>{fmt(sub)}</span>}
                                    </div>
                                    {extras.map((e, j) => (
                                      <div key={j} style={{ fontSize:11.5, color:'var(--lx-muted)', paddingLeft:18, display:'flex', justifyContent:'space-between', gap:10 }}>
                                        <span>+ {e.nombre}</span>
                                        <span>{e.precio > 0 ? `+${fmt(e.precio * cant)}` : 'Gratis'}</span>
                                      </div>
                                    ))}
                                  </div>
                                );
                          ...
                          {p.tipo === 'domicilio' && direccionAlt && (
                            <div style={{ fontSize:12, color:'var(--lx-muted)', marginBottom:6, display:'flex', alignItems:'center', gap:4 }}>
                              🛵 Entrega en: <strong style={{ color:'var(--lx-text)' }}>{direccionAlt}</strong>
                            </div>
                          )}
                          {motivoRechazo && (estNorm === 'cancelado') && (
                            <div style={{ margin:'8px 0 10px', padding:'10px 12px', borderRadius:10, background:'rgba(239,83,80,.1)', border:'1px solid rgba(239,83,80,.3)' }}>
                              <div style={{ fontSize:11, fontWeight:800, color:'#EF5350', marginBottom:2 }}>Comprobante rechazado</div>
                              <div style={{ fontSize:12.5, color:'var(--lx-text)' }}>{motivoRechazo}</div>
                            </div>
                          )}
                          {estNorm !== 'entregado' && estNorm !== 'cancelado' && estNorm !== 'anulado' && (
                          ...
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                            ...
                            <span style={{ fontWeight:700, fontSize:14, color:'var(--lx-green)' }}>{fmt(p.total)}</span>
                          </div>
