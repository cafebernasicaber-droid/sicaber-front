        ...
        <div className="insumos-card">
          {displayed.length === 0 ? (
            <div className="empty-state">
              ...
              {!query && hasPermiso('toppings', 'crear') && <button className="btn-add-first" onClick={() => setModal('new')}>Nuevo topping</button>}
            </div>
          ) : (
                  ...
                  {displayed.map(t => {
                    ...
                    return (
                      <tr key={t.id}>
                        ...
                        <td>
                          {hasPermiso('toppings', 'editar') ? (
                            <button className={`toggle-btn ${t.estado==='Activo'?'toggle-on':'toggle-off'}`} onClick={() => handleToggle(t.id)} title={t.estado}>
                              <span className="toggle-thumb"/>
                            </button>
                          ) : (
                            <span className={`toggle-btn ${t.estado==='Activo'?'toggle-on':'toggle-off'}`} style={{cursor:'default',opacity:0.6}}><span className="toggle-thumb"/></span>
                          )}
                        </td>
                        <td>
                          <div className="actions-group">
                            ...
                            {hasPermiso('toppings', 'eliminar') && <AnularButton onClick={() => setDel(t)}/>}
                          </div>
                        </td>
                      </tr>
                    );
