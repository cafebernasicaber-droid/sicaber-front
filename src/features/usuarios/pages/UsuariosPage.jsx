...
function ModalFormUsuario({ usuario, roles, tipoFijo, isSuperAdmin, onCreate, onUpdate, onClose }) {
  ...
  const handleSubmit = async e => {
    ...
    // Debe ser un local REAL de GET /locales: un valor legado como 'Local 1'
    // (que ya no existe en el catálogo) obliga a reasignar.
    const localReal = locales.find(l => l.nombre === form.sede);
    if (form.rol !== 'Administrador' && !localReal) {
    ...
    setLoading(true);
    const data = {
      nombre: form.nombre, username: form.username, correo: form.correo,
      password: form.password, rol: form.rol, sede: form.sede,
      // local_id explícito para que el backend no tenga que adivinarlo por
      // nombre (y nunca caiga a su default 'Local 1').
      local_id: localReal ? localReal.id : null,
    };
    try {
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {form.rol && form.rol !== 'Administrador' && (
                <div style={{ maxWidth: '50%', paddingRight: 6 }}>
                  ...
                  {(() => {
                    // El <select> SOLO ofrece locales reales (GET /locales).
                    // Un usuario con un local de una versión anterior
                    // ('Local 1'/'Local 2', ya inexistentes) no arrastra ese
                    // texto a la lista: queda sin selección y un aviso pide
                    // reasignarlo. Nunca se muestra "Local 1"/"Local 2".
                    const localValido = locales.some(l => l.nombre === form.sede);
                    const tieneLocalLegado = !localValido && !!form.sede && form.sede !== 'Ambos';
                    return (
                      <>
                        <select style={inputStyle}
                          value={localValido ? form.sede : ''} onChange={e => set('sede', e.target.value)}>
                          <option value="">Seleccionar local...</option>
                          {locales.map(l => <option key={l.id} value={l.nombre}>{l.nombre}</option>)}
                        </select>
                        {tieneLocalLegado ? (
                          <div style={{ fontSize: 11, color: '#C9A227', marginTop: 5, fontWeight: 600 }}>
                            ⚠ Este usuario tiene asignado un local que ya no existe. Elige uno de la lista para reasignarlo.
                          </div>
                        ) : (
                          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 5 }}>
