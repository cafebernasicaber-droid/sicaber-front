...
function ModalConfirm({ dev, venta, accion, onClose, onConfirm }) {
  ...
  // Rechazar exige un motivo: el backend lo valida y lo guarda en
  // devoluciones.motivo_rechazo. Antes el rechazo era un botón sin más, y
  // la devolución quedaba marcada como rechazada sin ninguna explicación.
  ...
  const confirmar = () => {
    ...
    if (limpio.length > LIMITES.MOTIVO) {
      setErrMotivo(`El motivo no puede superar los ${LIMITES.MOTIVO} caracteres.`);
      return;
    }
    onConfirm(limpio);
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e=>e.stopPropagation()}>
        ...
        {!esAprobar && (
          <div style={{textAlign:'left',marginTop:16}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',marginTop:4,fontSize:11}}>
              <span style={{color:'#E53935'}}>{errMotivo}</span>
              <span style={{color: enElTope(motivo, LIMITES.MOTIVO) ? '#E53935' : 'var(--text-muted)'}}>
                {contador(motivo, LIMITES.MOTIVO)}
              </span>
            </div>
            <p style={{fontSize:11.5,color:'var(--text-muted)',marginTop:8,marginBottom:0}}>
              Este motivo queda guardado y se muestra en el detalle de la devolución.
            </p>
          </div>
        )}
        ...
        <div className="modal-actions">
          ...
          {esAprobar
            ? <button className="btn-confirm-primary" onClick={confirmar}>✅ Sí, aprobar</button>
            : <button className="btn-confirm-danger" onClick={confirmar}>❌ Sí, rechazar</button>
          }
        </div>
      </div>
    </div>
...
export default function DevolucionesPage() {
  ...
  const filtradas = devs.filter(d => {
    ...
    const mq = !lq || String(getDevId(d)).includes(lq) || String(d.pedido_id).includes(lq) || (v?.cliente||'').toLowerCase().includes(lq) || (d.motivo||'').toLowerCase().includes(lq) || (d.motivo_rechazo||'').toLowerCase().includes(lq);
    ...
    <Layout>
      <div className="insumos-root">
        ...
        <div className="page-header" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between'}}>
          ...
          {hasPermiso('devoluciones', 'gestionar') && (
                    ...
                    return (
                      <tr key={getDevId(d)}>
                        ...
                        <td style={{maxWidth:160,fontSize:12,color:'var(--text-secondary)'}}>
                          <div style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} title={d.motivo}>{d.motivo}</div>
                          {/* Motivo del RECHAZO (distinto del motivo por el que se
                              pidió la devolución). Solo aparece en las rechazadas,
                              que son las únicas que lo tienen guardado. */}
                          {(d.estado === 'rechazada') && (d.motivo_rechazo || d.motivoRechazo) && (
                            <div title={d.motivo_rechazo || d.motivoRechazo}
                              style={{marginTop:5,padding:'4px 8px',borderRadius:6,background:'#FFEBEE',
                                borderLeft:'3px solid #C62828',color:'#B71C1C',fontSize:11.5,
                                overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                              ❌ Rechazada: {d.motivo_rechazo || d.motivoRechazo}
                            </div>
                          )}
                        </td>
                        ...
                        <td>
                          <div className="actions-group">
                            ...
                            {canRechazar && hasPermiso('devoluciones', 'gestionar') && (
