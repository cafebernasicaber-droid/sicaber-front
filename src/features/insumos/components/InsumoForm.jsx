...
const InsumoForm = ({ initialData, onSubmit, onCancel, isEditing, serverError, onManageCategorias, insumosExistentes = [] }) => {
  ...
  // El JWT real de este backend nunca trae "localId" (solo "sede", un
  // texto como "Local Villa Liliam") — así que para cualquier usuario que
  // NO pueda elegir el local a mano, hay que resolver su id real
  // buscando ese nombre dentro de la lista de locales ya cargada. Sin
  // esto, un cajero/bartender con sede fija nunca podría crear un insumo
  // (el backend exige local_id siempre).
  const localIdDesdeSede = (!isEditing && user?.sede && user.sede !== 'Ambos')
    ? (locales.find(l => l.nombre === user.sede)?.id ?? null)
    : null;
  ...
  const localActual = isEditing
    ...
    : puedeElegirLocal
      ...
      : (() => {
          const porId = localIdDesdeSede != null
            ? locales.find(l => String(l.id) === String(localIdDesdeSede))
            : null;
          return { id: localIdDesdeSede, nombre: porId?.nombre || (user?.sede && user.sede !== 'Ambos' ? user.sede : '') };
        })();
  ...
  const sinLocalAsignado = !isEditing && !puedeElegirLocal && !localActual.nombre && (localIdDesdeSede == null);
  ...
  const validate = (f = form) => {
    ...
    else if (sinLocalAsignado) errs.localId = 'Tu usuario no tiene un local de trabajo asignado. Pide a un administrador que te asigne uno.';
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...
      // El backend exige local_id siempre al crear un insumo (columna
      // NOT NULL). Si el usuario puede elegirlo a mano, se manda el que
      // seleccionó; si no, se manda el que se resolvió a partir de su
      // sede fija (localIdDesdeSede, ver arriba). Al editar nunca se
      // envía (el local es inmutable, y el backend lo rechaza si llega
      // distinto al ya guardado).
      ...(!isEditing
        ? { localId: Number(puedeElegirLocal ? form.localId : localIdDesdeSede) }
        : {}),
    });
  };
