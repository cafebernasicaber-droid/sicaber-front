...
export function EmpleadoModal({ initial, onClose, onSave, cargoFijo }) {
  ...
  const [form, setForm] = useState(initial ? {
    ...
    password:  '',
    sede:      initial.sede      || '',
    // 3 — local REAL asignado (tabla `locales`, FK), aparte de `sede`
    // (nombre en texto, legado). El backend ya acepta local_id en
  ...
  const handleSubmit = async e => {
    ...
    // Exige un local REAL (local_id de GET /locales): un valor legado como
    // 'Local 1' en `sede` sin `localId` no cuenta — hay que reasignarlo.
    if (esLoginCargo && !form.localId) { setError('Selecciona a qué local pertenece este empleado.'); return; }
          ...
          {esLoginCargo && (
            <div className="emp-form-row">
              <div className="emp-form-group" style={{gridColumn:'1/-1'}}>
                ...
                {(() => {
                  // El selector SOLO ofrece locales reales (GET /locales). Un
                  // empleado con un local de una versión anterior ('Local 1'/
                  // 'Local 2', que ya no existen) no arrastra ese texto a la
                  // lista: el <select> queda sin selección y un aviso pide
                  // reasignarlo — nunca se muestra "Local 1"/"Local 2".
                  const localValido = locales.some(l => String(l.id) === String(form.localId));
                  const tieneLocalLegado = !localValido && !!form.sede;
                  return (
                    <>
                      <select
                        value={localValido ? form.localId : ''}
                        onChange={e => {
                          const loc = locales.find(l => String(l.id) === e.target.value);
                          setForm({ ...form, sede: loc ? loc.nombre : '', localId: loc ? loc.id : '' });
                        }}>
                        <option value="">— Seleccionar —</option>
                        {locales.map(l => <option key={l.id} value={l.id}>{l.nombre}</option>)}
                      </select>
                      {tieneLocalLegado ? (
                        <div style={{fontSize:11,color:'#C9A227',marginTop:4,fontWeight:600}}>
                          ⚠ Este {form.cargo.toLowerCase()} tiene asignado un local que ya no existe. Elige uno de la lista para reasignarlo.
                        </div>
                      ) : (
...
function LocalesTab({ showOk }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // El backend responde 409 con un mensaje que dice EXACTAMENTE qué
      // registros tiene asociados el local (ver DELETE /locales/:id:
      // "No se puede eliminar: este local tiene 3 pedidos y 1 insumo
      // asociados..."). Se muestra ese texto tal cual en el modal, en vez
      // del genérico "Error en la solicitud". Se cubren las dos formas en
      // que api.js puede exponer el detalle: err.message y err.error.
      setDeleteError(err?.message || err?.error || 'No se pudo eliminar el local. Intenta de nuevo.');
    }
  };
    ...
    <div className="insumos-card">
      {modal && (
        <LocalFormModal
          ...
          onSave={() => { refresh(); setModal(null); showOk(modal === 'new' ? 'Local creado' : 'Local actualizado'); }}
        />
      )}
      {deleteTarget && (
        <div className="modal-overlay" onClick={() => { setDeleteTarget(null); setDeleteError(''); }}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <h3>{deleteError ? 'No se puede eliminar el local' : '¿Eliminar este local?'}</h3>
            <div className="modal-detail">"{deleteTarget.nombre}"</div>
            {deleteError ? (
              // Mensaje REAL del backend (409): dice exactamente qué registros
              // asociados impiden borrar el local.
              <div style={{ background:'rgba(229,57,53,0.12)',color:'#C62828',padding:'12px 14px',borderRadius:8,margin:'12px 0 0',fontSize:13,lineHeight:1.5,textAlign:'left',display:'flex',gap:8,alignItems:'flex-start' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink:0, marginTop:2 }}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            ...
            <div className="modal-actions">
              {deleteError ? (
                <button className="btn-cancel" onClick={() => { setDeleteTarget(null); setDeleteError(''); }}>Entendido</button>
              ) : (
                <>
...
export default function EmpleadosPage() {
  ...
  // 3 — filtro por local asignado (además de que el buscador de texto ya
  // encuentra por nombre de local, esto da un selector explícito con la
  // lista real de locales, igual que ya hace LocalFiltro en otros módulos).
  const [localFiltro, setLocalFiltro] = useState('Todos');
  const [localesFiltro, setLocalesFiltro] = useState([]);
  useEffect(() => {
    // Filtro por local: basta con los locales ACTIVOS y GET /locales es
    // público (sin 403). Antes usaba getAll() -> /locales/todos, que
    // requiere rol Administrador y disparaba un 403 al cargar la página
    // para cualquier rol limitado, aunque este filtro sea accesorio.
    localesService.getActivos().then(d => setLocalesFiltro(Array.isArray(d) ? d : [])).catch(() => setLocalesFiltro([]));
  }, []);
  ...
  if (localFiltro !== 'Todos') shownFiltered = shownFiltered.filter(e => (e.sede || '') === localFiltro);
  const shown = [...shownFiltered].sort((a, b) => Number(b.id) - Number(a.id));

  // 7 — "Limpiar filtros": resetea de un clic búsqueda + estado + local.
  const hayFiltrosActivos = !!query || estadoFiltro !== 'Todos' || localFiltro !== 'Todos';
  const limpiarFiltros = () => { setQuery(''); setEstadoFiltro('Todos'); setLocalFiltro('Todos'); };
    ...
    <Layout>
      <div style={{ position:'relative' }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:16 }}>
          ...
          {/* La pestaña "Locales" (y su GET /locales/todos, que exige rol
              Administrador) solo se ofrece si el rol tiene permiso sobre
              Locales — si no, ni siquiera se dispara la petición. */}
          {hasPermiso('locales', 'ver') && (
            <button
        ...
        {tab === 'locales' && hasPermiso('locales', 'ver') ? (
        ...
        <div className="insumos-toolbar">
          ...
          <select value={localFiltro} onChange={e => setLocalFiltro(e.target.value)}
            title="Filtrar empleados por local asignado"
            style={{ padding:'9px 12px', border:'1.5px solid var(--border-input)', borderRadius:8, fontSize:12.5, background:'var(--bg-input)', color:'var(--text-primary)', outline:'none', cursor:'pointer' }}>
            <option value="Todos">Todos los locales</option>
            {localesFiltro.map(l => <option key={l.id} value={l.nombre}>{l.nombre}</option>)}
          </select>
          {hayFiltrosActivos && (
            <button className="btn-limpiar-filtros" title="Limpiar filtros" onClick={limpiarFiltros}>
              ✕ Limpiar filtros
            </button>
          )}
                  ...
                  {shown.map(e => {
                    ...
                    return (
                      <tr key={e.id}>
                        ...
                        <td>
                          {hasPermiso('empleados', 'editar') ? (
                            <button className={`toggle-btn ${e.estado === 'Activo' ? 'toggle-on' : 'toggle-off'}`} onClick={() => toggleActivo(e.id)}>
                              <span className="toggle-thumb"/>
                            </button>
                          ) : (
                            <span className={`toggle-btn ${e.estado === 'Activo' ? 'toggle-on' : 'toggle-off'}`} style={{ cursor:'default', opacity:0.6 }}>
                              <span className="toggle-thumb"/>
                            </span>
                          )}
                        </td>
