          ...
          {esLoginCargo && (
            <div className="emp-form-row">
              <div className="emp-form-group" style={{gridColumn:'1/-1'}}>
                ...
                <select
                  value={form.localId || (form.sede ? '__legacy__' : '')}
                  onChange={e => {
                    const val = e.target.value;
                    if (val === '__legacy__') return; // opción de solo lectura, no cambia nada
                    const loc = locales.find(l => String(l.id) === val);
                  ...
                  {form.sede && !locales.some(l => String(l.id) === String(form.localId)) && (
                    <option value="__legacy__">{form.sede} (local antiguo, reasignar)</option>
                  )}
                </select>
...
export default function EmpleadosPage() {
  ...
  useEffect(() => {
    localesService.getAll().then(d => setLocalesFiltro(Array.isArray(d) ? d : [])).catch(() => setLocalesFiltro([]));
  }, []);
