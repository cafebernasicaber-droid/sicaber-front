...
import { mensajeErrorEstadoPedido, normalizarEstadoPedido, etiquetaEstadoPedido } from '../../../shared/utils/pedidoEstados';
...
const STATUS_CFG = {
  ...
  // Valores legados que aún pueden venir de la BD.
  en_preparacion: { label: 'En proceso', color: '#42A5F5', bg: '#E3F2FD' },
  listo:          { label: 'En camino',  color: '#00838F', bg: '#E0F7FA' },
};
...
function BartenderCard({ order, onStart, onReady, onDetail, onTomar }) {
  ...
  const cfgLabel = order.estado === 'pendiente' ? cfg.label : etiquetaEstadoPedido(order.estado, order.tipo);
  const isPending = order.estado === 'pendiente';
  const isPrep    = normalizarEstadoPedido(order.estado) === 'en_proceso';
  ...
  return (
    <div className={`bt-card ${isPrep ? 'bt-card--prep' : ''}`}>
      ...
      <div className="bt-card__actions">
        ...
        {isPrep && (
          <button className="bt-btn bt-btn--green" onClick={() => onReady(order.id, order.tipo)}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <polyline points="20 6 9 17 4 12"/>
            </svg>
            {order.tipo === 'domicilio' ? 'Enviar' : 'Preparado'}
          </button>
        )}
      </div>
    </div>
...
export default function BartenderPage() {
  ...
  const filtered = filter === 'all'
    ? orders
    : orders.filter(o => normalizarEstadoPedido(o.estado) === filter);
  ...
  const handleStart = useCallback((id) => {
    // 'en_proceso' es el valor real del backend para "en preparación"
    // (antes se mandaba 'en_preparacion', que el backend rechaza).
    pedidosService.cambiarEstado(id, 'en_proceso')
      .then(() => { refresh(); showToast('¡Preparación iniciada!'); })
      .catch((err) => showToast(mensajeErrorEstadoPedido(err, 'No se pudo actualizar el pedido.')));
  }, [refresh]);

  const handleReady = useCallback((id, tipo) => {
    pedidosService.cambiarEstado(id, 'en_camino')
      .then(() => {
        refresh();
        showToast(tipo === 'domicilio' ? '✓ Pedido en camino' : '✓ Pedido listo para recoger');
      })
      .catch((err) => showToast(mensajeErrorEstadoPedido(err, 'No se pudo actualizar el pedido.')));
  }, [refresh]);
      ...
      <aside className="bt-sidebar">
        ...
        <nav className="bt-sidebar__nav">
          ...
          <div className="bt-sidebar__info">
            ...
            <div className="bt-sidebar__info-row">
              <span>🔵</span> En proceso = estás preparando
            </div>
            <div className="bt-sidebar__info-row">
              <span>🟢</span> Preparado = pasa a entrega / recogida
            </div>
          </div>
        </nav>
