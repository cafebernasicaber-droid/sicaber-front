...
import { normalizarEstadoPedido, etiquetaEstadoPedido } from '../utils/pedidoEstados';
import './DomiciliosBell.css';

// Estados en los que un domicilio sigue "vivo" (no entregado ni cancelado).
// Se normaliza el estado del pedido antes de comparar, así que basta con los
// valores canónicos (los legados 'en_preparacion'/'listo' quedan cubiertos).
const ESTADOS_ACTIVOS = ['pendiente_verificacion', 'pendiente', 'en_proceso', 'en_camino'];
...
const DomiciliosBell = ({ onVerTodos }) => {
  ...
  const domicilios = pedidos
    .filter(p => p.tipo === 'domicilio' && ESTADOS_ACTIVOS.includes(normalizarEstadoPedido(p.estado)))
              ...
              domicilios.map(p => (
                <div key={p.id} className="dmb-item">
                  <div className="dmb-item__top">
                    ...
                    <span className={`dmb-item__estado dmb-item__estado--${normalizarEstadoPedido(p.estado)}`}>
                      {etiquetaEstadoPedido(p.estado, 'domicilio')}
                    </span>
                  </div>
