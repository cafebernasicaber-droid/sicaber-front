        ...
        try {
          ...
          setUser(prev => {
            ...
            const fresco = {
              ...
              // local_id real (locales.id) del local de trabajo del usuario —
              // lo usa el formulario de Insumos para mostrar (solo lectura) en
              // qué local quedará el insumo y para validar nombres duplicados
              // por local. El backend lo toma del JWT, aquí solo se refleja.
              localId:  data.local_id ?? prev.localId ?? null,
              // ¿Es el superadministrador? (usuarios.es_superadmin). Lo usa el
              // formulario de Insumos: un superadmin sin local_id fijo puede
              // elegir el local a mano con un selector, en vez del campo de
              // solo lectura. Se aceptan varios alias por si el backend lo
              // expone en camelCase.
              esSuperadmin: (data.es_superadmin ?? data.esSuperadmin ?? data.es_superadministrador ?? prev.esSuperadmin) || false,
  ...
  const login = async (username, password) => {
    try {
      ...
      const u = {
        ...
        // local_id real del local de trabajo (locales.id) — ver nota en el
        // refresco de /auth/me más abajo. Null para el Administrador (sede
        // 'Ambos', sin local fijo).
        localId:  data.usuario.local_id ?? null,
        // Superadministrador (usuarios.es_superadmin) — ver nota en /auth/me.
        esSuperadmin: (data.usuario.es_superadmin ?? data.usuario.esSuperadmin ?? data.usuario.es_superadministrador) || false,
      ...
      // Tarea 1, punto 1d: para los roles del panel se manda a '/admin' (no
      // a '/admin/dashboard' fijo). '/admin' -> HomeRedirect elige el primer
      // módulo al que el rol SÍ tenga permiso; un rol sin acceso a Dashboard
      // ya no cae en "acceso no autorizado" nada más entrar.
